...
export default function withContext<T>(
  ...
  return (Component: React.ElementType) => {
    ...
    |}> {
      /**
       * All components that subscribes to the context would re render even when props they are not
       * subscribing to change. This shouldComponentUpdate should make the subscriber only re render
       * when its props are changing
       */
